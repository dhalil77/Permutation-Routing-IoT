...
def run_until_mote_is_ready_for_app(sim_engine, mote):
    mote.roupe.original_action_receive_dio = mote.roupe.action_receiveDIO
    def new_action_receive_dio(self, packet):
        ...
        if (
            ...
            mote.roupe.original_action_receive_dio(packet)
            sim_engine.pauseAtAsn(sim_engine.getAsn() + 1)
            mote.roupe.action_receiveDIO = mote.roupe.original_action_receive_dio
        else:
            # it's not ready; do nothing
            pass
    mote.roupe.action_receiveDIO = types.MethodType(new_action_receive_dio, mote.roupe)
