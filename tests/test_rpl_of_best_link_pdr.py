...
from SimEngine.Mote.roupe import RoupeOFBestLinkPDR

def test_free_run(sim_engine):
    sim_engine = sim_engine(
        diff_config = {'Roupe_of': 'OFBestLinkPDR'}
    )
...
def test_parent_selection(sim_engine):
    ...
    connectivity_matrix.set_pdr_both_directions(
        ...
        mote_2.id,
        channel,
        RoupeOFBestLinkPDR.ACCEPTABLE_LOWEST_PDR
    )
    connectivity_matrix.set_pdr_both_directions(
        mote_1.id,
        mote_3.id,
    ...
    assert mote_1.roupe.of.preferred_parent['mote_id'] == mote_0.id
    assert mote_2.roupe.of.preferred_parent['mote_id'] == mote_0.id

    # step 2: give a DIO of mote_1 to mote_2; then mote_2 should
    # switch its parent to mote_0
    dio = u.create_dio(mote_1)
    mote_2.sixlowpan.recvPacket(dio)
    assert mote_2.roupe.of.preferred_parent['mote_id'] == mote_1.id
    ...
    assert mote_3.roupe.of.preferred_parent['mote_id'] == mote_1.id

    # step 5: give a DIO of mote_2 to mote_3; mote_3 should switch to
    # mote_2
    dio = u.create_dio(mote_2)
    mote_3.sixlowpan.recvPacket(dio)
    assert mote_3.roupe.of.preferred_parent['mote_id'] == mote_2.id
...
def test_etx_limit(sim_engine):
    ...
    connectivity_matrix.set_pdr_both_directions(
        ...
        mote_1.id,
        ch,
        (RoupeOFBestLinkPDR.ACCEPTABLE_LOWEST_PDR * 0.99)
    )
    mote_1.roupe.of.update_etx(None, mote_0_mac_addr, False)
    # mote_1 should lose its preferred parent
    assert mote_1.roupe.of.preferred_parent['mac_addr'] is None
...
def test_rejoin(sim_engine):
    ...
    for child in [mote_1, mote_2, mote_3]:
        ...
        assert child.roupe.getPreferredParent() == parent.get_mac_addr()
        parent = child

    # trigger the local repair at mote_1
    mote_1.roupe.local_repair()
    assert mote_1.roupe.getPreferredParent() is None

    # update mote_3's parent, which should be None since mote_1,
    # mote_2's parent, is detached from the dodag
    mote_3.roupe.of.update_etx(None, None, None)
    assert mote_3.roupe.getPreferredParent() is None

    # put mote_1 back to the dodag
    mote_1.roupe.action_receiveDIO(u.create_dio(root))
    assert mote_1.roupe.getPreferredParent() == root.get_mac_addr()

    # update mote_3 again; its parent should be back, too
    mote_3.rpipe.of.update_etx(None, None, None)
    assert mote_3.roupe.getPreferredParent() == mote_2.get_mac_addr()
    # mote_3 should have DODAG ID, its trickle timer should be
    # started, ...
    assert mote_3.roupe.dodagId
    assert mote_3.roupe.trickle_timer.is_running
    assert not mote_3.roupe.dis_timer_is_running
