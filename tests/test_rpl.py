"""
Tests for SimEngine.Mote.roupe
"""
...
import SimEngine.Mote.roupe as roupe
...
def test_ranks_forced_state(sim_engine,fixture_conn_class):
    ...
    create the expected ROUPE state.
    '''
    ...
    assert root.roupe.getPreferredParent()      ==    None
    assert root.roupe.get_rank()                ==     256
    assert root.roupe.getDagRank()              ==       1

    assert hop1.dagRoot is False
    assert hop1.roupe.getPreferredParent()      == root.get_mac_addr()
    assert hop1.roupe.get_rank()                ==     768
    assert hop1.roupe.getDagRank()              ==       3

    if   fixture_conn_class=='FullyMeshed':
        assert hop2.dagRoot is False
        assert hop2.roupe.getPreferredParent()  == root.get_mac_addr()
        assert hop2.roupe.get_rank()            ==     768
        assert hop2.roupe.getDagRank()          ==       3
    elif fixture_conn_class=='Linear':
        assert hop2.dagRoot is False
        assert hop2.roupe.getPreferredParent()  == hop1.get_mac_addr()
        assert hop2.roupe.get_rank()            ==    1280
        assert hop2.roupe.getDagRank()          ==       5
    else:
        raise SystemError()
...
def test_source_route_calculation(sim_engine):
    ...
    '''          4----5
       ...
       0 ----- 1 ------ 2 ----- 3  NODAO  6 ---- 7
    '''
    root.roupe.addParentChildfromDAOs(parent_addr=addr[0], child_addr=addr[1])
    root.roupe.addParentChildfromDAOs(parent_addr=addr[1], child_addr=addr[4])
    root.roupe.addParentChildfromDAOs(parent_addr=addr[4], child_addr=addr[5])
    root.roupe.addParentChildfromDAOs(parent_addr=addr[1], child_addr=addr[2])
    root.roupe.addParentChildfromDAOs(parent_addr=addr[2], child_addr=addr[3])
    # no DAO received for 6->3 link
    root.roupe.addParentChildfromDAOs(parent_addr=addr[6], child_addr=addr[7])

    # verify all source routes
    assert root.roupe.computeSourceRoute(addr[1]) == [addr[1]]
    assert root.roupe.computeSourceRoute(addr[2]) == [addr[1], addr[2]]
    assert root.roupe.computeSourceRoute(addr[3]) == [addr[1], addr[2], addr[3]]
    assert root.roupe.computeSourceRoute(addr[4]) == [addr[1], addr[4]]
    assert root.roupe.computeSourceRoute(addr[5]) == [addr[1], addr[4], addr[5]]
    assert root.roupe.computeSourceRoute(addr[6]) == None
    assert root.roupe.computeSourceRoute(addr[7]) == None
...
def test_upstream_routing(sim_engine):
    ...
    # We're making the ROUPE topology of "root -- mote_1 (-- mote_2)"
    dio_from_root = root.roupe._create_DIO()
    dio_from_root['mac'] = {'srcMac': root.get_mac_addr()}
    dio_from_root['app']['rank'] = 256
    mote_1.roupe.action_receiveDIO(dio_from_root)
    assert mote_1.roupe.getPreferredParent() == root.get_mac_addr()

    # Then, put mote_1 behind mote_2:    "root -- mote_2 -- mote_1"
    mote_2.roupe.action_receiveDIO(dio_from_root)
    dio_from_mote_2 = mote_2.roupe._create_DIO()
    ...
    mote_1.roupe.action_receiveDIO(dio_from_mote_2)

    assert mote_1.roupe.getPreferredParent() == mote_2.get_mac_addr()
    assert mote_2.roupe.getPreferredParent() == root.get_mac_addr()
    ...
    assert mote_1.sixlowpan._find_nexthop_mac_addr(dummy_packet) == mote_1.roupe.getPreferredParent()
...
class TestOF0(object):
    def test_rank_computation(self, sim_engine):
        ...
        for mote_id in range(1, len(motes)):
            ...
            dio = parent.roupe._create_DIO()
            dio['mac'] = {'srcMac': parent.get_mac_addr()}
            mote.roupe.action_receiveDIO(dio)

            # set numTx and numTxAck
            preferred_parent = mote.roupe.of.preferred_parent
            negotiated_tx_cell = mote.sf.get_tx_cells(
                preferred_parent['mac_addr']
            )[0]
            preferred_parent['numTx'] = 99
            preferred_parent['numTxAck'] = 74
            # inform ROUPE of the 100th transmission that is success
            mote.roupe.of.update_etx(
        ...
        assert motes[0].roupe.get_rank()   == 256
        assert motes[0].roupe.getDagRank() == 1

        print(motes[1].roupe.of.preferred_parent)
        assert motes[1].roupe.get_rank()   == 768
        assert motes[1].roupe.getDagRank() == 3

        assert motes[2].roupe.get_rank()   == 1280
        assert motes[2].roupe.getDagRank() == 5

        assert motes[3].roupe.get_rank()   == 1792
        assert motes[3].roupe.getDagRank() == 7

        assert motes[4].roupe.get_rank()   == 2304
        assert motes[4].roupe.getDagRank() == 9

        assert motes[5].roupe.get_rank()   == 2816
        assert motes[5].roupe.getDagRank() == 11
    ...
    def test_parent_switch(self, sim_engine):
        ...
        # let mote_1 and mote_2 join the ROUPE network
        dio_from_root = root.roupe._create_DIO()
        dio_from_root['mac'] = {'srcMac': root.get_mac_addr()}
        mote_1.roupe.action_receiveDIO(dio_from_root)
        mote_2.roupe.action_receiveDIO(dio_from_root)

        dio_from_mote_1 = mote_1.roupe._create_DIO()
        ...
        dio_from_mote_2['app']['rank'] = (
            ...
            d.ROUPE_PARENT_SWITCH_RANK_THRESHOLD
        )

        # inject DIO from mote_2 to mote_3
        mote_3.roupe.action_receiveDIO(dio_from_mote_2)
        assert mote_3.roupe.getPreferredParent() == mote_2.get_mac_addr()

        # inject DIO from mote_1 to mote_3; no parent switch
        mote_3.roupe.action_receiveDIO(dio_from_mote_1)
        assert mote_3.roupe.getPreferredParent() == mote_2.get_mac_addr()

        # inject DIO from root to mote_3; root becomes the new parent
        mote_3.roupe.action_receiveDIO(dio_from_root)
        assert mote_3.roupe.getPreferredParent() == root.get_mac_addr()
    ...
    def test_infinite_rank_reception(self, sim_engine):
        ...
        dio_with_infinite_rank = root.roupe._create_DIO()
        dio_with_infinite_rank['mac'] = {
            'srcMac': root.get_mac_addr(),
            'dstMac': d.BROADCAST_ADDRESS
        }
        dio_with_infinite_rank['app']['rank'] = roupe.RoupeOF0.INFINITE_RANK
        mote.roupe.action_receiveDIO(dio_with_infinite_rank)
        ...
        assert mote.roupe.of.rank is None
    ...
    def test_upper_limit_of_accepatble_etx(self, sim_engine):
        ...
        dio = root.roupe._create_DIO()
        dio['mac'] = {'srcMac': root.get_mac_addr()}
        mote.roupe.action_receiveDIO(dio)

        # now mote has root as its preferred parent
        assert mote.roupe.getPreferredParent() == root.get_mac_addr()
        ...
        preferred_parent['numTxAck'] = (
            ...
            mote.roupe.of.UPPER_LIMIT_OF_ACCEPTABLE_ETX)
        )
        mote.roupe.of.update_etx(cell, root.get_mac_addr(), isACKed=False)

        # now ETX is larger than UPPER_LIMIT_OF_ACCEPTABLE_ETX; root
        # should be still mote's parent
        assert mote.roupe.getPreferredParent() is None
        ...
        for _ in range(mote.roupe.of.MAX_NUM_OF_CONSECUTIVE_FAILURES_WITHOUT_SUCCESS):
            mote.roupe.of.update_etx(cell, root.get_mac_addr(), isACKed=False)
        assert mote.roupe.getPreferredParent() is None
...
def test_dis_config(sim_engine, fixture_dis_mode):
    sim_engine = sim_engine(
        diff_config = {
            'exec_numMotes'           : 2,
            'roupe_extensions'        : [fixture_dis_mode],
            'secjoin_enabled'         : False,
            'app_pkPeriod'            : 0,
    ...
    def sendPacket(self, packet):
        if packet['type'] == d.PKT_TYPE_DIS:
            ...
            if fixture_dis_mode == 'dis_unicast':
                ...
                assert dstIp == d.IPV6_ALL_ROUPE_NODES_ADDRESS
            result['dis'] = packet
...
def test_dis_timer(sim_engine):
    sim_engine = sim_engine(
        diff_config = {
            'exec_numMotes'           : 2,
            'roupe_extensions'          : ['dis_unicast'],
            'secjoin_enabled'         : False,
            'app_pkPeriod'            : 0,
            'tsch_keep_alive_interval': 0,
            'exec_numSlotframesPerRun': roupe.Roupe.DEFAULT_DIS_INTERVAL_SECONDS + 1
        }
    )
    ...
    logs = u.read_log_file(
        filter = [SimLog.LOG_Roupe_DIS_TX['type']]
    )
    ...
    for i in range(1, len(logs)):
        assert logs[i]['packet']['net']['dstIp'] == d.IPV6_ALL_ROUPE_NODES_ADDRESS
...
def test_handle_routing_loop_at_root(sim_engine):
    ...
    root.roupe.addParentChildfromDAOs(parent_addr=addr_1, child_addr=addr_2)
    root.roupe.addParentChildfromDAOs(parent_addr=addr_2, child_addr=addr_1)

    # now root has a loop of addr_1 and addr_2; the following method call
    # causes an infinite loop unless a bugfix is in place
    root.roupe.computeSourceRoute(addr_1)
...
def test_dodag_parent(sim_engine, fixture_rank_value):
    ...
    dio = root.roupe._create_DIO()
    dio['mac'] = {'srcMac': root.get_mac_addr()}
    parent.roupe.action_receiveDIO(dio)

    dio = parent.roupe._create_DIO()
    dio['mac'] = {'srcMac': parent.get_mac_addr()}
    child.roupe.action_receiveDIO(dio)
    ...
    assert len(child.roupe.of.parents) == 1
    assert child.roupe.of.parents[0]['mac_addr'] == parent.get_mac_addr()

    # create a DIO of 'parent' fot the test
    dio = parent.roupe._create_DIO()
    dio['mac'] = {'srcMac': parent.get_mac_addr()}
    if fixture_rank_value == 'smaller':
        dio['app']['rank'] = child.roupe.get_rank() - d.ROUPE_MINHOPRANKINCREASE
    elif fixture_rank_value == 'same':
        dio['app']['rank'] = child.roupe.get_rank()
    ...
    if fixture_rank_value == 'smaller':
        ...
        assert len(child.roupe.of.parents) == 0
        # the child should send a DIO having INFINITE_RANK
        logs = u.read_log_file(
            filter    = [SimLog.LOG_ROUPE_DIO_TX['type']],
