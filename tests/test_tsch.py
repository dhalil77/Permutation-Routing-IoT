...
def test_tx_cell_selection(
    ...
    if   destination == 'broadcast':
        packet['net']['dstIp'] = d.IPV6_ALL_ROUPE_NODES_ADDRESS
...
def test_retransmission_backoff_algorithm(sim_engine, cell_type):
    ...
    assert hop_1.roupe.dodagId is None
    dio = root.roupe._create_DIO()
    dio['mac'] = {'srcMac': root.get_mac_addr()}
    hop_1.roupe.action_receiveDIO(dio)
    assert hop_1.roupe.dodagId is not None
