...
from SimEngine import SimConfig,   \
                      ...
                      SimEngine,   \
                      Connectivity
from SimEngine.Mote.roupe import RoupeOFNone
    ...
    while list(state.values()).count('seen')<len(state):
        ...
        for child in list(state.keys()):
            ...
            if pdr_not_null(child,parent,engine):
                ...
                child.roupe.of = RoupeOFNone(child.roupe)
                child.roupe.of.set_preferred_parent(parent.get_mac_addr())
                # set child's rank
                child.roupe.of.set_rank(parent.roupe.get_rank()+512)
                # record the child->parent relationship at the root (for source routing)
                root.roupe.addParentChildfromDAOs(
