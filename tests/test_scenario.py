...
def roupe_check_all_node_prefered_parent(motes):
    """ Verify that each mote has a preferred parent """
    for mote in motes:
        if mote.dagRoot:
            continue
        else:
            assert mote.roupe.getPreferredParent() is not None

def roupe_check_all_node_rank(motes):
    """ Verify that each mote has a rank """
    for mote in motes:
        assert mote.roup.get_rank() is not None

def roupe_check_all_nodes_send_DIOs(motes):
    check_all_nodes_send_x(motes,'DIO')

def roupe_check_all_motes_send_DAOs(motes):
    senders = list(
        set([l['_mote_id']
...
def roupe_check_root_parentChildfromDAOs(motes):
    root = motes[0]
    ...
    assert root.roupe.parentChildfromDAOs == expected
...
def tsch_all_nodes_check_dedicated_cell(motes):
    ...
    for mote in motes:
        ...
        parent = mote.roupe.getPreferredParent()
...
def test_vanilla_scenario(
    ...
    roupe_check_all_node_prefered_parent(sim_engine.motes)
    roupe_check_all_node_rank(sim_engine.motes)

    # verify that all nodes are sending EBs, DIOs and DAOs
    tsch_check_all_nodes_send_EBs(sim_engine.motes)
    roupe_check_all_nodes_send_DIOs(sim_engine.motes)
    roupe_check_all_motes_send_DAOs(sim_engine.motes)

    # verify that root has stored enough DAO information to compute source routes
    roupe_check_root_parentChildfromDAOs(sim_engine.motes)
