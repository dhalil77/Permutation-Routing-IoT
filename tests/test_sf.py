...
class TestMSF(object):
    ...
    def test_msf(self, sim_engine):
        ...
        def do_nothing(self):
            pass
        mote.roupe._send_DIO = types.MethodType(do_nothing, mote)
        ...
        # 3.1 increase the following Roupe values in order to avoid invalidating
        # the root as a parent
        mote.roupe.of.MAX_NUM_OF_CONSECUTIVE_FAILURES_WITHOUT_ACK = 100
        mote.roupe.of.UPPER_LIMIT_OF_ACCEPTABLE_ETX = 100
        mote.roupe.of.MAXIMUM_STEP_OF_RANK = 100
    ...
    def test_parent_switch(self, sim_engine):
        ...
        dio = old_parent.roupe._create_DIO()
        dio['mac'] = {'srcMac': old_parent.get_mac_addr()}
        dio['app']['rank'] = 65535
        mote_1.roupe.action_receiveDIO(dio)
        # give a DIO from new_parent with a good rank
        dio = new_parent.roupe._create_DIO()
    ...
    def test_retry(self, sim_engine):
        ...
        logs = u.read_log_file(
            filter=[SimLog.LOG_ROUPE_CHURN['type']]
        )
        assert len(logs) == 2
