""" Routing Permutation Implementation
...
class Roupe(object):
    ...
    def getDagRank(self):
        if self.of.rank is None:
            ...
            return int(old_div(self.of.rank, d.ROUPE_MINHOPRANKINCREASE))
    ...
    def start(self):
        ...
        #trace.write("LISTE :" + str(self.listeGlobal)  +"\t") 
        #trace.close()

        if self.mote.dagRoot:
            self.dodagId = self.mote.get_ipv6_global_addr()
            self.of.set_rank(d.ROUPE_MINHOPRANKINCREASE)
            self.trickle_timer.start()
            # now start a new ROUPE instance; reset the timer as per Section 8.3 of
            # RFC 6550
            self.trickle_timer.reset()
        else:
            if self.settings.Roupe_of:
                # update OF with one specified in config.json
                of_class  = u'Roupe{0}'.format(self.settings.Roupe_of)
            ...
            if self.dis_mode != u'disabled':
                ...
                if self.dis_mode == u'dis_unicast':
                    ...
                    dstIp = d.IPV6_ALL_ROUPE_NODES_ADDRESS
                else:
    ...
    def indicate_preferred_parent_change(self, old_preferred, new_preferred):
        # log
        self.log(
            SimEngine.SimLog.LOG_ROUPE_CHURN,
            {
    ...
    def local_repair(self):
        ...
        assert (
            ...
            or
            (self.of.rank == d.ROUPE_INFINITE_RANK)
        )
        self.log(
            SimEngine.SimLog.LOG_ROUPE_LOCAL_REPAIR,
            {
                "_mote_id":        self.mote.id
            }
        )
    ...
    def action_receiveDIS(self, packet):
        self.log(
            SimEngine.SimLog.LOG_ROUPE_DIS_RX,
            {
                "_mote_id":  self.mote.id,
                "packet":    packet,
            }
        ...
        if self.dodagId is None:
            ...
            elif packet[u'net'][u'dstIp'] == d.IPV6_ALL_ROUPE_NODES_ADDRESS:
                # broadcast DIS
                self.trickle_timer.reset()
            else:
                # shouldn't happen
    ...
    def _get_dis_mode(self):
        if   u'dis_unicast' in self.settings.roupe_extensions:
            assert u'dis_broadcast' not in self.settings.roupe_extensions
            return u'dis_unicast'
        elif 'dis_broadcast' in self.settings.roupe_extensions:
            assert u'dis_unicast' not in self.settings.roupe_extensions
    ...
    def handle_dis_timer(self):
        self.send_DIS(d.IPV6_ALL_ROUPE_NODES_ADDRESS)
        self.start_dis_timer()
    ...
    def send_DIS(self, dstIp):
        ...
        self.log(
            SimEngine.SimLog.LOG_ROUPE_DIS_TX,
            {
    ...
    def _send_DIO(self, dstIp=None):
        ...
        # log
        self.log(
            SimEngine.SimLog.LOG_ROUPE_DIO_TX,
            {
                u'_mote_id':  self.mote.id,
                u'packet':    dio,
            }
        ...
        #trace = open("Dossier/FichierDeTrace.txt","a")
        #trace.write("SEND DIO LANCE " +str(self.mote.id) +"\n" )  
        #trace.close()
    ...
    def action_receiveDIO(self, packet):
        ...
        # log
        self.log(
            SimEngine.SimLog.LOG_ROUPE_DIO_RX,
            {
                u'_mote_id':  self.mote.id,
                u'packet':    packet,
            }
        ...
        #trace = open("Dossier/FichierDeTrace.txt","a")
        #trace.write(" RECEIVE DI0 LANCE \n" +str(self.mote.id) +"\n")  
        #trace.close()

        # handle the infinite rank
        if packet[u'app'][u'rank'] == d.ROUPE_INFINITE_RANK:
            if self.dodagId is None:
                # ignore this DIO
                return
            else:
    ...
    def _schedule_sendDAO(self, firstDAO=False):
        ...
        if self.settings.roupe_daoPeriod == 0:
        ...
        if firstDAO:
            ...
            asnDiff = int(math.ceil(
                old_div(random.uniform(
                    0.8 * self.settings.roupe_daoPeriod,
                    1.2 * self.settings.roupe_daoPeriod
    ...
    def _action_enqueueDAO(self):
        ...
        # log
        self.log(
            SimEngine.SimLog.LOG_ROUPE_DAO_TX,
            {
                u'_mote_id': self.mote.id,
                u'packet':   newDAO,
            }
    ...
    def action_receiveDAO(self, packet):
        ...
        # log
        self.log(
            SimEngine.SimLog.LOG_ROUPE_DAO_RX,
            {
                u'_mote_id': self.mote.id,
                u'packet':   packet,
            }
...
class RoupeOFBase(object):
    def __init__(self, roupe):
        self.roupe = roupe
        self.rank = None
    ...
    def reset(self):
        ...
        self.preferred_parent = None
        self.roupe.indicate_preferred_parent_change(
    ...
    def poison_roupe_parent(self, mac_addr):
        pass

class RoupeOFNone(RoupeOFBase):
...
class RoupeOF0(RoupeOFBase):
    ...
    def poison_roupe_parent(self, mac_addr):
        ...
        if neighbor:
            self._update_neighbor_rank(neighbor, d.ROUPE_INFINITE_RANK)
            self.rank = None
            self._update_preferred_parent()
    ...
    def _update_neighbor_rank_increase(self, neighbor):
        ...
        if step_of_rank is None:
            ...
            neighbor[u'rank_increase'] = step_of_rank * d.ROUPE_MINHOPRANKINCREASE
    ...
    def _update_preferred_parent(self):
        if (
            ...
            # difference is more than d.ROUPE_PARENT_SWITCH_RANK_THRESHOLD, we dump
        ...
        if new_rank is None:
            ...
            if rank_difference is not None:
                if d.ROUPE_PARENT_SWITCH_RANK_THRESHOLD < rank_difference:
                    new_parent = candidate
                    self.rank = new_rank
                else:
        ...
        if (
            ...
            # reset Trickle Timer
            self.roupe.trickle_timer.reset()
        elif (
                (new_parent is None)
                and
                (self.preferred_parent is not None)
            ):
            self.roupe.local_repair()
        else:
            # do nothing
            pass

class RoupeOFBestLinkPDR(RoupeOF0):
    ACCEPTABLE_LOWEST_PDR  = 1.0 / RoupeOF0.UPPER_LIMIT_OF_ACCEPTABLE_ETX
    INVALID_RSSI_VALUE = -1000
    NONE_PREFERRED_PARENT = {
        u'mac_addr': None,
        u'mote_id': None,
        u'rank': d.ROUPE_INFINITE_RANK,
        u'mean_link_pdr': 0,
        u'mean_link_rssi': INVALID_RSSI_VALUE
    }

    def __init__(self, roupe):
        super(RoupeOFBestLinkPDR, self).__init__(roupe)
        self.preferred_parent = self.NONE_PREFERRED_PARENT
        self.neighbors = []
        self.path_pdr = 0

        # short hand
        self.mote = self.roupe.mote
        self.engine = self.roupe.engine
    ...
    def parents(self):
        ...
        for neighbor in self.neighbors:
            ...
            while parent_mote.dagRoot is False:
                assert parent_mote.roupe.of.preferred_parent
                parent_id = parent_mote.roupe.of.preferred_parent[u'mote_id']
    ...
    def reset(self):
        super(RoupeOFBestLinkPDR, self).reset()
        self.preferred_parent = self.NONE_PREFERRED_PARENT
        self.neighbors = []
    ...
    def update(self, dio):
        ...
        if dio[u'app'][u'rank'] == d.ROUPE_INFINITE_RANK:
            if neighbor is None:
                # do nothing
                pass
            else:
    ...
    def poison_roupe_parent(self, mac_addr):
        ...
        if neighbor is not None:
            neighbor[u'rank'] = d.ROUPE_INFINITE_RANK
        self._update_preferred_parent()
        # send a broadcast DIS to collect neighbors which we've not
        # noticed
        self.roupe.send_DIS(d.IPV6_ALL_ROUPE_NODES_ADDRESS)
    ...
    def _calculate_rank(neighbor):
        ...
        if (
            ...
            rank_increase = step_of_rank * d.ROUPE_MINHOPRANKINCREASE
    ...
    def _update_preferred_parent(self):
        if self.parents:
            ...
            new_rank = d.ROUPE_INFINITE_RANK
