...
class Sixlowpan(object):
    ...
    def sendPacket(self, packet):
        ...
        # mark a downward packet like 'O' option in ROUPE Option defined by RFC
        # 6553
        ...
        if goOn:
            if (
                ...
                sourceRoute = self.mote.roupe.computeSourceRoute(packet[u'net'][u'dstIp'])
                if sourceRoute==None:
        ...
        if goOn:
            if  (
                ...
                if   packet[u'type'] in [d.PKT_TYPE_JOIN_REQUEST,d.PKT_TYPE_JOIN_RESPONSE]:
                    ...
                    self.mote.roupe.action_receiveDAO(packet)
                elif packet[u'type'] == d.PKT_TYPE_DIO:
                    self.mote.roupe.action_receiveDIO(packet)
                elif packet[u'type'] == d.PKT_TYPE_DIS:
                    self.mote.roupe.action_receiveDIS(packet)
                elif packet[u'type'] == d.PKT_TYPE_DATA:
                    self.mote.app.recvPacket(packet)
                    self.mote.app.receive()
                    self.mote.app.resume()
                ...
                preferred_parent_mac_addr = self.mote.roupe.getPreferredParent()
                if (
                        (
                    ...
                    if (
                        ...
                        # reset Trickle timer
                        self.mote.roupe.trickle_timer.reset()
                        return
                    else:
    ...
    def _find_nexthop_mac_addr(self, packet):
        ...
        if (dst_ip_addr.words[0] & 0xFF00) == 0xFF00:
            ...
            if self.mote.roupe.dodagId is None:
                ...
                mac_addr = self.mote.roupe.getPreferredParent()
