...
class ConnectivityMatrixBase(object):
    ...
    def __init__(self, connectivity):
        # local variables
        self.mote_id_list = [mote.id for mote in connectivity.engine.motes]
        self.x = [mote.id for mote in connectivity.engine.motes]
...
class ConnectivityMatrixGroupingAlgorithm(ConnectivityMatrixBase):
    """
    Regroupment algorithm:
    Node are divided into groups
    All nodes can hear all nodes with PDR=100% into each group
    Leanar connectivity between first node of each group
    """
    ...
    def _additional_initialization(self):
        ...
        G = []
        X = self.mote_id_list
        for i in range(int(math.sqrt(len(X)))):
            G.append([X.pop(0)])
        
        while len(X) > 0:
            # we find min array and we insert elt into
            minIndex = 0
            for i in range(1, len(G)):
                if len(G[minIndex]) > len(G[i]):
                    minIndex = i
            G[minIndex].append(X.pop(0))
